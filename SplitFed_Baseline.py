...
def build_model(n_class: int, input_shape: tuple, layers: list[int]):
    ...
    for i, units in enumerate(layers):
       # x = keras.layers.Dense(units, activation ='relu', kernel_regularizer=keras.regularizers.L1(l1=1e-5), name=f"dense{i}")(x)
...
def train_splitfed(
    ...
    return history, avg_history, (model_server, model_client)
# %%
# Experiment
def split_dataset(
    k: int,
...
def experiment(exp_name:str, cut_layer:int|str):
    ...
    history, avg_history, (server_trained, client_trained) = train_splitfed(
        model_server = server,
        model_client = client,
    ...
    acc_list = []
    for i in range(10):
        loss, acc = global_model.evaluate(client_data[i][2])
        loss_list.append(loss)
        acc_list.append(acc)
    acc = np.average(acc_list)
    loss = np.average(loss_list)
    print(f"[{exp_name}] Global Test Performance: {acc =}, {loss=}")

    return history, avg_history, acc
# %%
history, avg_history, acc = experiment(exp_name='alpha_0p3', cut_layer='dense3')
